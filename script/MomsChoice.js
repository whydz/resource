...
var userCookie = ($.isNode() ? require('./MomsChoice_token.json') : $.getdata(ckName)) || '';
let userList = [];
let userIdx = 0;
let userCount = 0;
// 调试
...
class UserInfo {
  ...
  async refreshToken(token) {
    try {
      ...
      $.isNode() ? require('fs').writeFileSync('./MomsChoice_token.json', JSON.stringify(userCookie)) : $.setdata(JSON.stringify(userCookie), ckName);
      $.log(`🎉 刷新 token 成功\n`)
